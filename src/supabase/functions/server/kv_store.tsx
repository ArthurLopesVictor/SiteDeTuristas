...
// View at https://supabase.com/dashboard/project/ckwavnkxqicrhktqmblw/database/tables
...
export const mset = async (keys: string[], values: any[]): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_a5c257ac").upsert(keys.map((k, i) => ({ key: k, value: values[i] })));
  if (error) {
    throw new Error(error.message);
  }
};
...
export const mdel = async (keys: string[]): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_a5c257ac").delete().in("key", keys);
  if (error) {
    throw new Error(error.message);
  }
};
